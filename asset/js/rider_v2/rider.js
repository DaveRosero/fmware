$(document).ready(function () {
  ...
  function fetchOrders() {
    $.ajax({
      url: "/model-order", // Your combined endpoint for both online and POS orders
      type: "GET",
      dataType: "json",
      success: function (data) {
        const { onlineOrders, posOrders } = data;
        ...
        // Filter POS orders similarly
        const filteredPOSOrders = posOrders.filter(
          (order) =>
            order.status.toLowerCase() === "pending" && // Status is pending
            (!order.rider_id || order.rider_id === null) && // No assigned rider
            (order.paid.toLowerCase() === "paid" ||
        ...
        // Combine both filtered orders
        const allOrders = [...filteredOnlineOrders, ...filteredPOSOrders];
  ...
  function displayOrders(orderList) {
    ...
    orderList.forEach((order) => {
      // Determine the type of order and set the appropriate fields
      const orderType = order.order_ref ? "Online" : "POS";
      const orderRef = order.order_ref || order.pos_ref || "N/A";
