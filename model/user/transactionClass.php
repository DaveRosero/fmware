...
class Transaction extends User
{
    ...
    public function getPreparedCount($user_id)
    {
        $query = 'SELECT COUNT(*) FROM orders WHERE user_id = ? AND status = "prepared"';
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('i', $user_id);
        if ($stmt) {
            if ($stmt->execute()) {
    ...
    public function getClaimedCount($user_id)
    {
        $query = 'SELECT COUNT(*) FROM orders WHERE user_id = ? AND status = "claimed"';
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('i', $user_id);
        if ($stmt) {
            if ($stmt->execute()) {
    ...
    public function getPreparedOrders($user_id)
    {
        $query = 'SELECT orders.id,
                            orders.order_ref,
                            orders.gross,
    ...
    public function getClaimedOrders($user_id)
    {
        $query = 'SELECT orders.id,
                            orders.order_ref,
                            orders.gross,
    ...
    public function displayHistory($status, $user_id)
    {
        ...
        if ($status === 'prepared') {
            $this->getPreparedOrders($user_id);
        }

        if ($status === 'claimed') {
            $this->getClaimedOrders($user_id);
        }
    }
}
